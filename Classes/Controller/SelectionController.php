...
class SelectionController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function getPidAndInit() {
		...
		if( !($this->cObj->data['pages'] == '') ) {
			...
			$res5 = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid',
					'pages',
	...
	public function contentAction() {
		...
		foreach ($pidsForeach as $pid) {
			...
			// 
			$res5 = $GLOBALS['TYPO3_DB']->exec_SELECTquery('pid, uid, header, tstamp',
					'tt_content',
	...
	public function pagesAction() {
		...
		foreach ($pidsForeach as $pid) {
			...
			$res5 = $GLOBALS['TYPO3_DB']->exec_SELECTquery('pid, uid, title, subtitle, abstract, description',
					'pages',
		...
		if (intval($GLOBALS['TSFE']->sys_language_uid) > 0) {
			// andere Sprache?
			foreach ($pids as $pid) {
				$res5 = $GLOBALS['TYPO3_DB']->exec_SELECTquery('pid, title, subtitle, abstract, description',
		...
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($res5)>0) {
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res5)){
				$uid = $row['uid'];
				$pid = $row['pid'];
				$dataArray[$pid]['elements'][$uid] = array();
	...
	public function newsAction() {
		...
		foreach ($pidsForeach as $pid) {
			...
			$res5 = $GLOBALS['TYPO3_DB']->exec_SELECTquery('tt_news.uid AS newsid, tt_news.title AS newstitle, tt_news.datetime AS newsdate, cat.uid AS catid, cat.title AS cattitle',
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($res5)>0) {
				while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res5)){
					if (!is_array($dataArray[$row['catid']])) {
						$dataArray[$row['catid']] = array();
						$dataArray[$row['catid']]['news'] = array();
					}
					$dataArray[$row['catid']]['header'] = $row['cattitle'];
					$dataArray[$row['catid']]['news'][$row['newsid']] = array();
	...
	public function camaligaAction() {
		...
		foreach ($pidsForeach as $pid) {
			...
			// News+Kategorien holen
			$res5 = $GLOBALS['TYPO3_DB']->exec_SELECTquery('tx_camaliga_domain_model_content.uid AS camid, tx_camaliga_domain_model_content.title AS camtitle, tx_camaliga_domain_model_content.shortdesc, tx_camaliga_domain_model_content.longdesc, tx_camaliga_domain_model_content.link, cat.uid AS catid, cat.title AS cattitle',
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($res5)>0) {
				while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res5)){
					if (!is_array($dataArray[$row['catid']])) {
						$dataArray[$row['catid']] = array();
						$dataArray[$row['catid']]['camaliga'] = array();
					}
					$camid = $row['camid'];
					$dataArray[$row['catid']]['header'] = $row['cattitle'];
					$dataArray[$row['catid']]['camaliga'][$camid] = array();
